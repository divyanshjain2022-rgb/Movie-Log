...
import { useDeleteMovie, useFormulaParams, useMovie, useUpdateMovie } from "@/hooks";
...
import type { RewatchSibling } from "@/lib/server/movie-detail-data";

interface MovieDetailClientProps {
  id: string;
  initialMovie: MovieWithRelations;
...
export function MovieDetailClient({ id, initialMovie, rewatches }: MovieDetailClientProps) {
  const router = useRouter();
  const { movie, isLoading, error, refetch } = useMovie(id, initialMovie);
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-4">
        ...
        {/* Rewatch History (siblings fetched server-side) */}
        {(() => {
